...
class GoogleMapViewState extends State<GoogleMapView> with TickerProviderStateMixin {
  ...
  /// Callback para tap em cluster: zoom in INTELIGENTE para mostrar os markers.
  /// 
  /// Em vez de dar zoom cego (que pode jogar markers dispersos para fora da tela),
  /// calculamos o bounding box dos N eventos mais próximos do centro do cluster
  /// e ajustamos a câmera para caber todos eles.
  void _onClusterTap(LatLng position, int count) {
    ...
    // 1. Encontrar os 'count' eventos mais próximos do centro do cluster.
    // Usamos os eventos atuais que estão renderizados na tela (antes da câmera mover).
    // Esses eventos JÁ ESTÃO na lista 'widget.viewModel.events'.
    final events = widget.viewModel.events;
    
    if (events.isEmpty) {
       _zoomToPosition(position, count);
       return;
    }

    // Heurística de segurança: se temos muito mais eventos na tela do que no cluster,
    // o 'sort' garante que pegamos os mais próximos.
    // Se count for exagerado (bug), clampamos ao total.
    final safeCount = count.clamp(1, events.length);

    // Calcular distâncias quadráticas (evita sqrt)
    ...
    // 2. Calcular bounds que engloba esses eventos
    double minLat = 90.0;
    double maxLat = -90.0;
    double minLng = 180.0;
    double maxLng = -180.0;
    ...
    // Verificar se é um "ponto único" (todos markers na mesma posição exata)
    final latSpan = (maxLat - minLat).abs();
    final lngSpan = (maxLng - minLng).abs();
    final isPoint = latSpan < 0.0001 && lngSpan < 0.0001;

    if (isPoint) {
       // Se estão todos no mesmo lugar, bounds não funciona (zoom infinito).
       // Usamos zoom cego incremental.
       _zoomToPosition(position, count);
    } else {
       ...
       // 3. Mover câmera para caber os eventos com padding
       // Padding de 80px garante que markers não fiquem colados na borda,
       // evitando que sejam cortados pela lógica de viewport.
       controller.animateCamera(CameraUpdate.newLatLngBounds(bounds, 80.0));
    }
  }

  void _zoomToPosition(LatLng position, int count) {
      // Zoom mais agressivo para clusters pequenos
      final zoomIncrement = count <= 3 ? 3.0 : 2.0;
      final targetZoom = (_currentZoom + zoomIncrement).clamp(0.0, 18.0);
      _mapController?.animateCamera(CameraUpdate.newLatLngZoom(position, targetZoom));
  }
  ...
  await manager.updateClusters(zoomLevel: _currentZoom);
    ...
    for (final m in clustered) {
      ...
      final clusterPin = await _getClusterPinWithEmoji(count, clusterEmoji);
  ...
  // Reduzido para 70ms para ser mais responsivo, mas com segurança da fila serializada
  ...
  // AGORA 3.0 (antes 2.0) para "salvar" markers de clusters largos que ficariam
  // fora do buffer ao dar zoom in, e alinhar melhor com buckets de cache (0.1 deg).
  static const double _viewportBoundsBufferFactor = 3.0;
  ...
  void initState() {
    ...
    // ✅ Removido registro direto no initState.
    // Agora gerenciado via TickerMode em didChangeDependencies para suportar IndexedStack.
    // MapNavigationService.instance.registerMapHandler(...)
    
    // 🔥 RESTAURADO: Registro de segurança no initState para deep links de cold start
  ...
  void didChangeDependencies() {
    ...
    // ✅ Gerenciar visibilidade no IndexedStack
    // Isso evita processar navegação quando o mapa está oculto (outra aba)
    // e garante que o handler seja registrado ao voltar para a aba.
    final isVisible = TickerMode.of(context);
    if (isVisible) {
      // debugPrint('👁️ [GoogleMapView] Visível - Registrando handler de navegação');
      _registerMapServices();
    } else {
      // debugPrint('🙈 [GoogleMapView] Oculto - Removendo handler de navegação');
      _unregisterMapServices();
    }
  }

  /// Centraliza o registro de serviços do mapa para evitar duplicação de código
  ...
  void _onMapCreated(GoogleMapController controller) async {
    ...
    // ✅ REGISTRAR HANDLERS NOVAMENTE (Garantia física)
    // Isso garante que se o initState falhou ou se o TickerMode estava off,
    // o mapa pronto assuma a responsabilidade.
    _registerMapServices();
  ...
  Future<void> _handleCameraIdleDebounced() async {
    ...
    try {
      ...
      final visibleRegion = await _mapController!.getVisibleRegion();
      widget.viewModel.setVisibleBounds(visibleRegion, zoom: _currentZoom);
      ...
      // CORREÇÃO: Usar bounds EXPANDIDO para a query principal também.
      // Isso garante que o MapDiscoveryService retorne eventos na "zona de gordura" (buffer),
      // evitando que markers na borda (ou parte de clusters de borda) sumam/pisquem
      // quando o MapDiscovery filtra estritamente pelo bounds passado.
      // 
      // Antes: queryBounds = visibleRegion (estrito) -> markers na borda sumiam.
      // Agora: queryBounds = expandedBounds (2.0x buffer) -> markers da borda persistem.
  ...
  Future<BitmapDescriptor> _getClusterPinWithEmoji(int count, String emoji) async {
  ...
  void _updateClustersRealtime() {
    // 1. Marcar que precisamos de um update
    _hasPendingClusterUpdate = true;

    if (!mounted) return;

    // 2. Se já tem um thread de processamento rodando, ele vai pegar a flag na próxima volta.
    if (_isClusterUpdateRunning) return;

    // 3. Se já tem um timer de debounce agendado, deixa ele disparar.
    if (_realtimeClusterDebounce?.isActive ?? false) return;
    
    // 4. Agenda o processamento
    _realtimeClusterDebounce = Timer(_realtimeClusterDebounceDuration, () {
      if (!mounted) return;
      // Inicia a fila de processamento serializado
      unawaited(_processClusterUpdateQueue());
    });
  }

  /// Processa updates de cluster serialmente.
  /// Garante que nunca chamamos manager.updateClusters() em paralelo na mesma instância.
  Future<void> _processClusterUpdateQueue() async {
    if (_isClusterUpdateRunning) return;
    _isClusterUpdateRunning = true;

    try {
      // Loop enquanto houver pendências (consumindo a flag)
      // Usamos while para garantir que se chegar novo pedido durante o processamento,
      // ele será atendido logo em seguida com o zoom mais recente.
      while (_hasPendingClusterUpdate && mounted) {
        // Consumir a flag antes de começar

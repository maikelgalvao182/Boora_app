...
class MarkersClusterManager {
  ...
  Future<void> updateClusters({required double zoomLevel}) async {
    ...
    if (radius == 0.0) {
      _clusteredMarkers = List<Marker>.from(_markers);
      return;
    }

    final markersSnapshot = List<Marker>.from(_markers);
    _clusteredMarkers = await _createClusters(markersSnapshot, radius);
  }
  ...
  Future<List<Marker>> _createClusters(
    ...
    final markersSnapshot = List<Marker>.from(markers);
    List<Marker> clusters = [];
    Set<Marker> processed = Set<Marker>();

    for (var marker in markersSnapshot) {
      ...
      for (var otherMarker in markersSnapshot) {
